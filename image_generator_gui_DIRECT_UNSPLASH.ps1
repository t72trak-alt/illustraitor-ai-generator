...
function Create-GUI {
    ...
    # Заполняем стили
    $styles = @(
        "🔮 Реализм", "🎨 Импрессионизм", "🌌 Сюрреализм", "🌀 Абстракциониism",
    ...
    # Поиск изображений в Unsplash (прямой доступ как в Chrome расширении)
    $btnSearchUnsplash.Add_Click({
        ...
        try {
            # Прямой запрос к Unsplash API как в Chrome расширении
            ...
            if ($response.results -and $response.results.Count -gt 0) {
                ...
                $global:generatedImageUrl = $imageUrl
                $global:currentSource = "unsplash"
                
                Show-Message "✅ Найдено изображение! ($($response.results.Count) результатов)" "Success"
                $script:btnDownload.Enabled = $true
                
                # Показываем информацию о изображении
                $photoInfo = "Автор: $($response.results[0].user.name)"
                Show-Message $photoInfo "Info"
